...
class SendPoints(Node):
	def __init__(self, points):
		...
		# Used for controlling when and how the replanning happens
		self.map_seen_once = False
		self.map_callback_count = 0
		self.map_replan_period = 8
	...
	def _start_action_client(self):
		...
		if self.next_goal_index == 0: # no change on this necessary
		...
		if self.next_goal_index >= len(self.goal_points):
			...
			# self.get_logger().info("") # consider printing the full text of the minecraft end credits to console
			# ... i just think it would be really funny if thats what our robot did when it finishes
			return
	...
	def map_callback(self, map_msg : OccupancyGrid):
		...
		# functionally the same idea as a PGM, defining wall, free space & our frontier.
		...
		for p in all_unseen_pts:
			already_tried = False
			for tried in self.tried_goal_points:
				dist = np.sqrt((p[0] - tried[0])**2 + (p[1] - tried[1])**2)
				if dist <= self.tried_goal_radius:
					already_tried = True
					break
		...
		if not self.map_seen_once:
			need_new_plan = True
		
		elif self.completed_all_goals() or len(self.goal_points) == 0:
			need_new_plan = True
		
		# The final goal point in image coords
		elif len(self.goal_points) > 0:
			current_goal_image = self.from_map_to_image(map_msg=map_msg, pt_xy=self.goal_points[-1]) 

			if not (0 <= current_goal_image[0] < map_msg.info.width and 0 <= current_goal_image[1] < map_msg.info.height):
				need_new_plan = True
			elif not is_free(im_thresh, current_goal_image):
				need_new_plan = True
		
		elif self.map_callback_count % self.map_replan_period == 0:
			need_new_plan = True
		
		if not need_new_plan:
			return
		
		if len(filtered_unseen_pts) == 0:
			self.map_seen_once = True
			return

		goal_loc_in_image = find_best_point(im_thresh, filtered_unseen_pts, robot_current_loc_in_image)

		if goal_loc_in_image is None:
			self.map_seen_once = True
			return
		...
		self.tried_goal_points.append(goal_loc_in_image)
...
def main(args=None):
	...
	points = []
	# TODO: replace this with the points generated by map_callback().
