...
# resetting the homeboy to blank
# this is important when you are stupid and do it wrong
...
# ... stamp: sec (seconds since 1970)
# ... frame_id: {string}, the name of the coordinate frame like /base_link or /odom, /laser etc
...
class Lab3Driver(Node):
	def __init__(self, threshold=0.6): # was 0.2
		...
		self.target_marker = None # watch where we define this to understand type/data structure

		#---------------------------- Lab 2 publishers and subscribers:--------------------------------------#
		...
		# this comes up as a visualization_msgs/marker message--how to read it in the method/class?
	...
	def close_enough(self): # NOTE: DONE
		""" Return true if close enough to goal. This will be used in action_callback to stop moving toward the goal
		@ return true/false """

		if self.target is None: # no target, so can't be close enough
			return False
		# calculate distance to target
		dist = self.distance_to_target()
		return dist < self.threshold # is True when True - et voila!
		# is this bad form? it's fun to do since the output matches the logic comparison, but might
		# ... not be the best for readability. Thoughts?
	...
	def get_obstacle(self, scan):
		...
		# from min distances of front, left, and right:
		# ... determine if there's an obstacle (presumably in front only, L/R not in path of collision)
		...
		theta = np.linspace(angle_min, angle_max, num_readings) # this modularizes further so we don't have to hardcode the +/- pi/2 rad FOV!

		# lab 1 useful holdovers:
		bot_padding = 0.40 # meters, extra wiggle room
		bot_width = 0.38 + bot_padding # meters
		angle_front_limit = np.arcsin(bot_width)

		# three useful zones of scans:
		front_mask = np.abs(theta) < angle_front_limit # bool array
		left_mask = theta >= angle_front_limit # another boolArray
		right_mask = theta <= -angle_front_limit

		# get min distance of scan
		def min_dist_from_scan(mask):
			readings = np.array(scan.ranges)[mask] # L/F/R
			return np.min(readings) if readings.size > 0 else scan.range_max # max scan range - don't hardcode!

		min_f = min_dist_from_scan(front_mask)
		min_l = min_dist_from_scan(left_mask)
		min_r = min_dist_from_scan(right_mask)
		
		# SUGGESTED SPEEDS: these are CONTROLS, not INDICATORS. 
		bot_vel = 1 # how fast SHOULD we move?  
		bot_turn = 0.0 # how fast SHOULD we turn?
		# ... to AVOID the obstacle.

		obstacle_too_close_threshold = 0.5 # meters
		obstacle_in_front = min_f < obstacle_too_close_threshold

		if obstacle_in_front: 
			bot_vel = 0.2*tanh(min_f - 0.5)
			if min_f < 0.5:
				bot_vel = 0.0 # hard stop if we get realllly close

			# if less immediate shit on the right, turn right!
			if min_l < min_r:
				bot_turn = -1 # turn right
			else:
				bot_turn = 1 

		if min_l < obstacle_too_close_threshold: # if there's an object on the left:
			if not obstacle_in_front:
				bot_turn = -0.1 # turn a little bit right
				bot_vel = 1
			else:
				bot_turn = -10

		if min_r < obstacle_too_close_threshold: # if there's an object on the right:
			if not obstacle_in_front:
				bot_turn = 0.1 # turn a little bit left
				bot_vel = 1 # and go forwards
			else:
				bot_turn = 1
		
		# adding in a measure of the distance from the obstacle for signal prioritization
		obst_dist = min_f # smallest distance in front of us == the obstacle


		return obstacle_in_front, bot_vel, bot_turn, obst_dist
	...
	def get_twist(self, scan):
		...
		current_dist = self.distance_to_target()

		# --- STUCK DETECTION LOGIC ---
		# Check if distance changed by more than 1cm
		if abs(current_dist - self.last_dist) > 0.01:
			self.last_dist = current_dist
			self.last_dist_time = current_time
		
		# If distance hasn't changed for 5 seconds and we aren't already reversing
		time_stuck = (current_time - self.last_dist_time).nanoseconds / 1e9
		if time_stuck > 5.0 and not self.is_reversing:
			self.get_logger().warn("Robot stuck! Initiating reverse maneuver.")
			self.is_reversing = True
			self.reverse_start_time = current_time

		# Handle the reversing state
		if self.is_reversing:
			time_since_reverse = (current_time - self.reverse_start_time).nanoseconds / 1e9
			if time_since_reverse < 1.0:  # Reverse for 2 seconds
				t.twist.linear.x = -0.2  # Set to negative max speed
				t.twist.angular.z = 0.0
				return t
			else:
				self.is_reversing = False
				self.last_dist_time = current_time # Reset timer so we don't loop instantly
		# -----------------------------
		...
		# 1: get angle, dist to waypoint
		angle_to_target = atan2(self.target.point.y, self.target.point.x)
		# this is MUCH simpler than it looks because you NEVER leave the robot's reference frame! Draw the FBD diagram out
		# ... it shows why we don't need global localization to get this waypoint.

		# distance_to_target = self.distance_to_target()
		# let's not name a variable the same name as a function... we don't need that kind of stress..

		# 2: obstacle avoidance with get_obstacle():
		is_obstacle, cmd_velx, cmd_turn, distance_to_obstacle  = self.get_obstacle(scan)
		# is_obstacle (in front of bot?): T/F
		# cmd_velx: if no, zero. If yes, slow accordingly.
		# cmd_turn: if no obst., don't turn. If YES, turn left or right.
		
		# 3: merge behaviors: 
		speed_to_goal = 0.4 * tanh(self.distance_to_target())
		t.twist.linear.x = min(speed_to_goal, cmd_velx) # set the smaller of the two for finer logic control
		# ... now we deal with rotation:
		target_turn = 1.0 * tanh(angle_to_target) # if any of this gives us shit, re-import as numpy.tanh not the deault python math lib
		t.twist.angular.z = 0.5*target_turn + 0.5*cmd_turn # attenuate inpute from both as it comes in
		#if math.copysign(1, target_turn) != math.copysign(1, cmd_turn):
		#	t.twist._angular.z = 1.0 # what is this, rads per second?
			# TODO: force this value for T seconds until the robot turns exactly 180 degrees


		# Weight = 1.0 at 0.5m, Weight = 0.0 at 2.0m
        # We use clip to ensure the weight stays in [0, 1]
		#weight = np.clip((2.0 - distance_to_obstacle) / (2.0 - 0.5), 0.0, 1.0)
		#t.twist.angular.z = (weight * 0.7* cmd_turn) + ((1 - weight) * 0.3* target_turn)

		# 4: refine and (actually) attenuate:
