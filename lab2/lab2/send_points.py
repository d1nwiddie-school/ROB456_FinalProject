...
# commenting the HELL out of this until I understand it like the back of the palm of my yesterday's hand or whatever 
...
class SendPoints(Node):
	def __init__(self, points):
		...
		# do we need to localize the robot OR completely rely on feedback from the action server to determine how we travel to the waypoint?
		...
		self.mutex = Lock()
		# what is a mutex? I get the idea, but is that an RCLPY thing or a python idiom?
		...
		# okay, so this is us creating/instantiating the action client, right?

		# Save the points for when we startup
		self.current_point = 0
		self.points = [p for p in points]
		# what trips me the hell up here is that this is a 1D point and not a 2D (x,y) tuple. How does that work, and why is it set up like this?
		# ... I also could just need to read further down, in all fairness ;p

		# Parameters that hold the current state of the action client
		self._send_goal_future = None # what would the data type be for this?
	...
	def _start_action_client(self):
		...
		# is this just good housekeeping or would there be a currently running timer at this point?

		if self.current_point == 0: # okay, zero is just the placeholder for <haven't started yet.>
			# Wait for driver to start
			self.get_logger().info("Start driver.py to get started")
			self.action_client.wait_for_server()
			# 
		
		if self.current_point >= len(self.points): # if the index would be greater than the number of all waypoints: ...
		# self.current_point is an INDEX, NOT a value. 
		# ... remember that .get_logger().info() is LITERALLY just a better print command. Use it frequently.
		...
		if self.current_point == 0:
			...
			# makes a markerArray, then:
			# ... locks the points while we:
			# ... ... add markers in the 'odom' frameID (this is for rviz and what else?)
			# ... ... set pretty params to make it look nice (these are the blue dots and lines in rviz) - they are dots, but we connect them with lines.
			# ... ...  loop through self.points which looks a bit like 
			# ... ... self.points = [(x0,y0), (x1,y1), ..., (xF, yF)].
			# TLDR: MAKES DOTS AND LINES IN RVIZ

		# send the current goal
		pt = self.points[self.current_point] # index the list of tuples
		# ... result will be a tuple (xi, yi) to play with. CARTESIAN (good!)
		# ... NOTE: self.current_point has NOTHING to do with the robot's localization. This is NOT robotGroundTruth or a representation of the bot's spot in the map.
