...
def convert_image(im, wall_threshold, free_threshold):
    ...
    # this is the infinity bit from lecture
...
def dijkstra(im, robot_loc=(0, 0), goal_loc=(0, 0)):
    ...
    while priority_queue:
        ...
        visited[current_node_ij] = (visited_triplet[0], visited_triplet[1], True)

        for adjacent_node in eight_connected(current_node_ij):
            # confirm map boundary
            if not (0 <= adjacent_node[0] < im.shape[1] and 0 <= adjacent_node[1] < im.shape[0]):
                continue
            # check if we can even move there
            if not is_free(im, adjacent_node):
                continue

            # cost penalty math
            is_diagonal = adjacent_node[0]!=current_node_ij[0] and adjacent_node[1]!=current_node_ij[1]
            move_cost = 1.414 if is_diagonal else 1.0
            new_distance = distance_to_current_node + move_cost

            # case where node is NEW to search or a shorter path is found during:
            if adjacent_node not in visited or new_distance < visited[adjacent_node][0]: 
                visited[adjacent_node] = (new_distance, current_node_ij, False)
                # reflect this priority in the queue
    ...
    if not goal_loc in visited:
        ...
        def closest_key(n):
            return np.sqrt((n[0]-goal_loc[0])**2 + (n[1]-goal_loc[1])**2)
        closest_node = min(visited.keys(), key=closest_key)
